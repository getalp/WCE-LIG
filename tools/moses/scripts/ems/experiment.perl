...
sub define_step {
	...
	next if &define_template($i);
	    ...
	    #TienNLe enable above command 2014.05.16
	}
	elsif ($DO_STEP[$i] =~ /^EVALUATION:(.+):ter$/) {
	    #&define_evaluation_ter($1);
	    #TienNLe enable above command 2014.05.16
	}
	elsif ($DO_STEP[$i] =~ /^EVALUATION:(.+):terp$/) {
	    #&define_evaluation_terp($1);
	    #TienNLe enable above command 2014.05.17
	}
