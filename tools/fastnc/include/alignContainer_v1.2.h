...
template <class Tr, class Th, class P> class AlignDTW
{
	protected:
		...
		int *BacktraceRef;
		int *BacktraceHyp;
		void  RemoveDuplicates(vector<Th> &Hyp, int OriRef, int OriHyp);
...
template <class Tr, class Th, class P> void AlignDTW<Tr,Th,P>::PrintBestAlign(vector<Th> &Hyp)
{
	...
	while (AlignDTW<Tr,Th,P>::SubInsDel.size())
	{
		...
		/*if (tmp1 == 0)	cerr <<"ok\t";
		else if (tmp1 == 1) cerr <<"ins\t";
		else if (tmp1 == 2) cerr <<"del\t";
		else cerr <<"sub\t";*/
		...
		if ((tmp1 == 0 || tmp1 == 1) && tmp2 >= 0 && tmp3 >=0) 
		{
			PrintRefOrHyp(Reference[tmp2], Hyp[tmp3]);
			//else if (tmp2 >= 0) PrintRefOrHyp(Reference[tmp2], NULL);
			//else PrintRefOrHyp(NULL, NULL);
			cout <<endl;
		}
		else if ((tmp1 == 0 || tmp1 == 1) && tmp3 >= 0) 
		{
			PrintRefOrHyp(NULL, Hyp[tmp3]);
			cout <<endl;
		}
	}
}
...
template <class Tr, class Th, class P> void AlignDTW<Tr,Th,P>::RemoveDuplicates(vector<Th> &Hyp, int OriRef, int OriHyp)
{
	int i;
	int Current = -2;
	float DistMax=-1;
	...
	for (i = AlignDTW<Tr,Th,P>::SubInsDel.size()-1; i > 0; i--)
	{
		//if (HypVec[i] >=0) cerr <<"current : "<<Hyp[HypVec[i]]->Line<<" ( "<<SubInsDel[i] <<" )"<<endl;
		...
		if (HypVec[i] != -1)
		{
			...
			if (SubInsDel[i] == 0 || SubInsDel[i] == 1 || SubInsDel[i] == 2)
			{
				SubInsDel[i] = 2;
				...
				{
					Current = HypVec[i];
					if (HypVec[i] >= 0 && RefVec[i] >=0)
						DistMax = CalculerDistance(Reference[RefVec[i]], Hyp[HypVec[i]]);
					else DistMax = -1;
					Good = i;
				}
				else
				{
					...
					if (HypVec[i] >= 0 && RefVec[i] >=0)
						tmp = CalculerDistance(Reference[RefVec[i]], Hyp[HypVec[i]]);
					else tmp = -1;
...
template <class Tr, class Th, class P> void AlignDTW<Tr,Th,P>::BackTrack(vector<Th> &Hyp, int OriRef, int OriHyp)
{
	...
	while (i > 0 && j > 0)
	{
		...
		if (Score >= Up && Score >= Left)
		{
			...
			/*if (Diag >= Left && Diag >= Up) i--, j--;
			//if (Up >= Diag && Up >= Left) i--;
			else if ( Up > Diag && Up > Left) j--;
			else*/ i--,j--;
		}
	...
	//RemoveDuplicates(Hyp, OriRef, OriHyp);
}
