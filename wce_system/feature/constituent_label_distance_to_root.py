...
def feature_distance_to_root(file_input_path, file_corpus_format_row, file_output_path):
    ...
    """
    str_message_if_not_existed = "Not Existed file output from TreeTagger: " + file_input_path
...
#**************************************************************************#
def feature_distance_to_root_threads(file_input_path, file_corpus_format_row, file_output_path, current_config):
    """
    Getting distances from leaves to root
    ...
    for line in file_reader:
        ...
        file_writer.write("\n") # empty line for seperating the "lines"
        #number_of_words_in_sentence = 0
        #print("Sentence processed %d." %number_of_sentence)
...
def feature_constituent_label(file_input_path, file_corpus_format_row, file_output_path):
    ...
    #check existed paths
    if not os.path.exists(file_input_path):
        raise TypeError('Not Existed file output from TreeTagger: '+ file_input_path)
...
def feature_constituent_label_threads(file_input_path, file_corpus_format_row, file_output_path, current_config):
    """
    Getting constituent label of each word
...
#**************************************************************************#
def replace_parenthesis_for_english_threads(file_input_path, file_output_path, current_config):
    """
    Replace parenthesis to brackets
...
def generate_constituent_tree_threads(file_input_path, target_language, file_output_constituent_tree_temp_path, current_config, config_end_user):
    ...
    grammar_path = ""
    l_threads = []
        #ts = threading.Thread(target=feature_constituent_label_get_list_distance_to_root_null_link_threads , args=( current_config.TARGET_REF_TEST_FORMAT_ROW+"."+str(l_inc), target_language, current_config.CONSTITUENT_TREE_TEMP+"."+str(l_inc), current_config.DISTANCE_TO_ROOT+"."+str(l_inc), current_config.CONSTITUENT_LABEL+"."+str(l_inc), current_config, config_end_user))
        #l_threads.append(ts)
        #ts.start()
    ...
    for l_inc in range(1,current_config.THREADS+1):
        if target_language == current_config.LANGUAGE_FRENCH: # the best for French :)
            ...
            script_path = config_end_user.TOOL_GET_CONSTITUENT_FR
            str_export_path = os.path.dirname(script_path) + "/../"
            command_line = script_path + " " + file_input_path +"."+str(l_inc) + " " + file_output_constituent_tree_temp_path +"."+str(l_inc) + " " + str_export_path
            ...
            replace_parenthesis_for_english_threads(file_input_path+"."+str(l_inc), file_input_path_temp+"."+str(l_inc), current_config)
            ##va cap nhat duong dan file_input_path thanh duong dan chi dinh san trong configuration
            #file_input_path = file_input_path_temp
            ...
            # $2: input file
            # $3: output file
            command_line = script_path + " " + grammar_path + " " + file_input_path_temp +"."+str(l_inc) + " " + file_output_constituent_tree_temp_path +"."+str(l_inc)
        #end if
        ...
        #export BONSAI=/home/lent/Téléchargements/a_moses/bonsai_v3.2
        print (command_line)
        ts = threading.Thread(target=call_script, args=( command_line, script_path))
        l_threads.append(ts)
        ts.start()
        time.sleep(0.5)
    for myT in l_threads:
...
#**************************************************************************#
def feature_constituent_label_get_list_distance_to_root_null_link_threads( file_input_path, target_language, file_output_constituent_tree_temp_path, file_output_distance_to_root_path, file_output_constituent_label_path, current_config, config_end_user):
    """
    Getting the features Constituent Label & Distance to Root & NULL link
    ...
    """
    #Buoc 1: Tao Constituent Tree
    generate_constituent_tree_threads(file_input_path, target_language, file_output_constituent_tree_temp_path, current_config, config_end_user)
    #Buoc 2: Distance to Root
        #ts = threading.Thread(target=feature_constituent_label_get_list_distance_to_root_null_link_threads , args=( current_config.TARGET_REF_TEST_FORMAT_ROW+"."+str(l_inc), target_language, current_config.CONSTITUENT_TREE_TEMP+"."+str(l_inc), current_config.DISTANCE_TO_ROOT+"."+str(l_inc), current_config.CONSTITUENT_LABEL+"."+str(l_inc), current_config, config_end_user))
        #l_threads.append(ts)
        #ts.start()
    ...
    l_threads = []
    for l_inc in range(1,current_config.THREADS+1):
        ts = threading.Thread(target=feature_distance_to_root_threads, args=( file_output_constituent_tree_temp_path+"."+str(l_inc), file_input_path+"."+str(l_inc), file_output_distance_to_root_path+"."+str(l_inc), current_config))
        l_threads.append(ts)
        ts.start()
    for myT in l_threads:
        myT.join()   
    ...
    l_threads = []
    for l_inc in range(1,current_config.THREADS+1):
        ts = threading.Thread(target=feature_constituent_label_threads, args=(file_output_constituent_tree_temp_path+"."+str(l_inc), file_input_path+"."+str(l_inc), file_output_constituent_label_path+"."+str(l_inc), current_config))
        l_threads.append(ts)
        ts.start()
    for myT in l_threads:
        myT.join()   
